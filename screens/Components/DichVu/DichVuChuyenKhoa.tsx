...
const DichVuChuyenKhoaScreen = () => {
  const route = useRoute();
  const { chuyenKhoaId } = route.params;
  ...
  const fetchDichVu = async () => {
    try {
      ...
      const response = await axios.get(`${BASE_URL}/api/DichVu/get-all-dich-vu-theo-loai-theo-chuyen-khoa?chuyenKhoaId=${chuyenKhoaId}`, {
  ...
  const handleSearch = async () => {
    try {
      const token = await AsyncStorage.getItem('data');
      const response = await axios.get(`${BASE_URL}/api/DichVu/search-dich-vu?searchKey=${search}&chuyenKhoaId=${chuyenKhoaId}`, {
  ...
  const renderService = ({ item }: { item: DichVuType }) => (
    <TouchableOpacity 
      ...
      onPress={() => navigation.navigate('ChiTietDichVu', { dichVuId: item.maDichVu, loaiDichVu: item.maLoaiDichVu })}
    >
      ...
      <Text style={styles.servicePrice}>{item.gia?.toLocaleString('vi-VN') || 'N/A'} đ</Text>
    </TouchableOpacity>
  );
...
export default DichVuChuyenKhoaScreen;
